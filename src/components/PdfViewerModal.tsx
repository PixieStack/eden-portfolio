...
export default function PdfViewerModal({ title, src, onClose }: PdfViewerModalProps) {
  ...
  const versionedSrc = `${src}${src.includes("?") ? "&" : "?"}v=20260830-2`;
  ...
  return createPortal(
    ...
    >
      <div className="flex h-[96dvh] w-full max-w-[96rem] flex-col overflow-hidden rounded-xl border border-white/10 bg-surface shadow-2xl shadow-black/60 sm:h-[94vh] sm:w-[96vw] sm:rounded-2xl">
        ...
        <div className="min-h-0 flex-1 bg-black/25 p-2.5 sm:p-4 lg:px-7 lg:py-5">
          <iframe
            src={`${versionedSrc}#page=1&zoom=page-width&toolbar=1&navpanes=0&pagemode=none`}
            title={`${title} PDF`}
