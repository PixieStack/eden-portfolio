...
export default function BadgeGroupCard({ group }: BadgeGroupCardProps) {
  return (
    <article className={`card overflow-hidden border border-white/10 ${group.featured ? "lg:col-span-2" : ""}`}>
      <div className="flex flex-col gap-4 border-b border-white/5 bg-gradient-to-r from-purple/10 to-transparent p-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-purple">
            <Sparkles size={17} />
            <span className="text-xs font-semibold uppercase tracking-[0.18em]">Verified badges</span>
          </div>
          <h3 className="mt-2 font-display text-2xl font-semibold text-white">{group.issuer}</h3>
      ...
      <div className={`grid gap-3 p-4 sm:p-6 ${group.featured ? "md:grid-cols-2" : ""}`}>
        {group.badges.map((badge) => (
          <a
            ...
            className="group flex min-h-24 items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.025] p-3 transition hover:-translate-y-0.5 hover:border-primary/35 hover:bg-primary/5"
            aria-label={`Open ${badge.name} badge credential`}
          >
            <div className="grid h-20 w-20 flex-none place-items-center overflow-hidden rounded-xl bg-white p-1.5">
              <img
      ...
      {group.profileUrl && (
        <div className="border-t border-white/5 px-6 py-4">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-sm font-semibold text-purple transition hover:text-white"
          >
